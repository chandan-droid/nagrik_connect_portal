...
import {
  Shield, Mail, Lock, User, Phone, ArrowRight, Eye, EyeOff,
  CheckCircle2, Users, BarChart3, AlertCircle
} from 'lucide-react';
...
type LoginRole = 'citizen' | 'admin' | 'officer';

const roleRouteMap: Record<LoginRole, string> = {
  citizen: '/citizen', admin: '/admin', officer: '/officer',
};

const roleConfig: Record<LoginRole, { label: string; icon: React.ElementType; desc: string; color: string; bg: string }> = {
  citizen: {
    label: 'Citizen', icon: Users, desc: 'File & track grievances',
    color: 'text-secondary', bg: 'bg-secondary/10 border-secondary/30',
  },
  officer: {
    label: 'Officer', icon: CheckCircle2, desc: 'Manage & resolve cases',
    color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/10 border-blue-500/30',
  },
  admin: {
    label: 'Admin', icon: BarChart3, desc: 'Analytics & oversight',
...
export default function AuthPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await signIn({ email: loginForm.email, password: loginForm.password, role: loginRole });
    setLoading(false);
    if (error) {
      toast({ title: 'Login Failed', description: error, variant: 'destructive' });
    } else {
      toast({ title: `Welcome back!`, description: `Signed in as ${roleConfig[loginRole].label}` });
      navigate(roleRouteMap[loginRole]);
    }
  };
          ...
          <Tabs defaultValue="login" className="w-full">
            ...
            <TabsContent value="login">
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div className="form-field">
                  ...
                  {errors.email && <p className="form-error"><AlertCircle className="w-3 h-3" />{errors.email}</p>}
                </div>
                ...
                <div className="form-field">
                  ...
                  {errors.password && <p className="form-error"><AlertCircle className="w-3 h-3" />{errors.password}</p>}
                </div>
                ...
                <Button type="submit" className="w-full h-11 gradient-primary text-white font-semibold shadow-md" disabled={loading}>
                  {loading ? (
                    ...
                    <><ArrowRight className="w-4 h-4 mr-1" /> Sign In as {roleConfig[loginRole].label}</>
                  )}
                </Button>
              </form>
            </TabsContent>
            ...
            <TabsContent value="signup">
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                <div className="form-field">
                  ...
                  <div className="relative mt-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => { setSignupForm({ ...signupForm, fullName: e.target.value }); setErrors({ ...errors, fullName: '' }); }}
                    />
                  </div>
                  {errors.fullName && <p className="form-error"><AlertCircle className="w-3 h-3" />{errors.fullName}</p>}
                </div>
                ...
                <div className="form-field">
                  ...
                  <div className="relative mt-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => { setSignupForm({ ...signupForm, email: e.target.value }); setErrors({ ...errors, email: '' }); }}
                    />
                  </div>
                  {errors.email && <p className="form-error"><AlertCircle className="w-3 h-3" />{errors.email}</p>}
                </div>
                ...
                <div className="form-field">
                  <Label className="form-label">Phone <span className="text-muted-foreground font-normal">(optional)</span></Label>
                  <div className="relative mt-1.5">
                ...
                <div className="form-field">
                  ...
                  <div className="relative mt-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => { setSignupForm({ ...signupForm, password: e.target.value }); setErrors({ ...errors, password: '' }); }}
                    />
                  </div>
                ...
                <Button type="submit" className="w-full h-11 bg-secondary text-white hover:bg-secondary/90 font-semibold shadow-md" disabled={loading}>
                  {loading ? (
                    ...
                    <><ArrowRight className="w-4 h-4 mr-1" /> Create Account</>
                  )}
                </Button>
